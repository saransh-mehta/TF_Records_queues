'''
Here we will encapsulate our model in a function called create_model
this will help us in choosing between custom activation function as well as datasets for
training and testing separately
'''
import tensorflow as tf

def create_model(images, labels, NUM_CLASSES, activation = None):

	if activation == 'custom':
		activation = lambda x: tf.nn.relu(x) - 0.02 * tf.nn.relu(-x)
	else:
